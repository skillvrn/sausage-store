pipeline {
    ...
    stages {
        ...
        stage('Notification to channel dvs_14') {
            steps {
                sh 'curl -X POST -H \'Content-type: application/json\' --data \'{"chat_id": "-1001823350512", "text": "Данил Кузнецов собрал приложение."}\' https://api.telegram.org/bot5933756043:AAE8JLL5KIzgrNBeTP5e-1bkbJy4YRoeGjs/sendMessage' // Отправляем сообщение в телеграм канал уведомлений.
            }
        }
    }

    post { 
        // после всей сборки чистим за собой
        cleanup {
            cleanWs()
        }
    }
